...
| GET | `/api/v1/files/{path}/versions` | 파일 버전 목록 | §9 |
| POST | `/api/v1/files/{path}/versions/{n}/restore` | 버전 복원 | §9 |
| POST | `/api/v1/shares` | 공유 생성 | §10 |
| GET | `/api/v1/shares` | 내가 공유한 목록 | §10 |
| GET | `/api/v1/shares/{id}` | 공유 상세 | §10 |
| PUT | `/api/v1/shares/{id}` | 공유 수정 | §10 |
| DELETE | `/api/v1/shares/{id}` | 공유 삭제 | §10 |
| GET | `/api/v1/shared-with-me` | 나에게 공유된 목록 | §10 |
| POST | `/api/v1/upload-url` | 업로드 WebDAV URL 발급 | §11 |
| POST | `/api/v1/download-url` | 다운로드 WebDAV URL 발급 | §11 |
| GET | `/s/{token}` | 공유 링크 정보 (비인증) | §12 |
| GET | `/s/{token}/download` | 공유 링크 다운로드 (비인증) | §12 |
...
### 14.1 Portal Drive UI (iframe)에서 API 호출
